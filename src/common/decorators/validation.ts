...
export class IsStrongPasswordConstraint implements ValidatorConstraintInterface {
  ...
  defaultMessage(args: ValidationArguments) {
    ...
    if (!/[^A-Za-z0-9]/.test(value)) return 'Пароль должен содержать минимум один специальный символ';
...
export class IsValidFullNameConstraint implements ValidatorConstraintInterface {
  validate(fullName: string, args: ValidationArguments) {
    ...
    return typeof fullName === 'string' && fullName.length >= 2 && fullName.length <= 50 && fullNameRegex.test(fullName);
  }

  defaultMessage(args: ValidationArguments) {
    const value = args.value as string;
    if (!value) return 'Полное имя обязательно';
    if (value.length < 2) return 'Полное имя должно содержать минимум 2 символа';
    if (value.length > 50) return 'Полное имя должно быть короче 50 символов';
    if (!/^[a-zA-Z\s'-]+$/.test(value)) return 'Полное имя может содержать только буквы, пробелы, дефисы и апострофы';
